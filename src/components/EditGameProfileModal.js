...
export default function EditGameProfileModal({ visible, profile, onClose, onSave }) {
  ...
  useEffect(() => {
    if (visible && profile) {
      setTitle(profile.title || profile.name || '');
      setSelectedSystem(profile.system || 'SNES');
      setCoverAsset(null);
      setManualAssets([]);
      setMapAsset(null);
      setIsSaving(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Save Error', 'Failed to update game profile.');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <Modal visible={visible} animationType="slide" transparent={false} onRequestClose={onClose}>
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Edit Game Profile</Text>
          ...
          <View style={styles.slotCard}>
            <View style={styles.slotInfo}>
              ...
              <Text style={styles.slotSubtitle}>
                {coverAsset
                  ? `New: ${coverAsset.name}`
                  : profile.coverUri || profile.coverUrl
                  ? 'Cover artwork attached'
                  : 'No cover image'}
              </Text>
            </View>
            <TouchableOpacity style={styles.slotBtn} onPress={handlePickCover}>
              <Text style={styles.slotBtnText}>{coverAsset ? '✓ Updated' : 'Replace'}</Text>
            </TouchableOpacity>
          </View>
            ...
            <View style={styles.slotInfo}>
              ...
              <Text style={styles.slotSubtitle}>
                {manualAssets.length > 0
                  ? `New: ${manualAssets.length} file(s)`
                  : profile.manual
                  ? `${profile.manual.pageCount || 1} pages attached`
                  : 'No manual attached'}
              </Text>
            </View>
            <TouchableOpacity style={styles.slotBtn} onPress={handlePickManual}>
              <Text style={styles.slotBtnText}>
                {manualAssets.length > 0 ? `✓ (${manualAssets.length})` : 'Replace'}
              </Text>
            </TouchableOpacity>
          </View>
            ...
            <View style={styles.slotInfo}>
              ...
              <Text style={styles.slotSubtitle}>
                {mapAsset
                  ? `New: ${mapAsset.name}`
                  : profile.map
                  ? 'High-res map attached'
                  : 'No map attached'}
              </Text>
            </View>
            <TouchableOpacity style={styles.slotBtn} onPress={handlePickMap}>
              <Text style={styles.slotBtnText}>{mapAsset ? '✓ Updated' : 'Replace'}</Text>
            </TouchableOpacity>
          </View>
